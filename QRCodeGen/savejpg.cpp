...
void load_bitmap(char *bitmap_name, WORD *Ximage_original, WORD *Yimage_original)
{
 ...
 fopen_s(&fp_bitmap, bitmap_name,"rb");
...
void SaveJPEGFile(char *szBmpFileNameIn, char* szJpgFileNameOut)
{
	char BMP_filename[64];
	char JPG_filename[64];
 	strcpy_s(BMP_filename, szBmpFileNameIn);
	strcpy_s(JPG_filename,  szJpgFileNameOut);
	...
	fopen_s(&fp_jpeg_stream, JPG_filename,"wb");
	init_all();
...
void WriteToFile(const char *filename)
{
  ...
  fopen_s(&fp_jpeg_stream, filename,"wb");

  init_all();

  //the original image dimensions
  SOF0info.width = wImage;
  SOF0info.height = hImage;
...
void setImageWidth(const WORD w)
{
  wImage = Ximage = w;
}

void setImageHeight(const WORD h)
{
  hImage = Yimage = h;
}
